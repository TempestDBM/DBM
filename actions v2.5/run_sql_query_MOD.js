module.exports = {
  ...
  fields: ['storage', 'stringifyOutput', 'varName', 'hostname', 'port', 'username', 'password', 'database', 'query', 'path', 'otype', 'source_conn_storage', 'source_conn_varName', 'store_source_conn_storage', 'store_source_conn_varName', 'debugMode', 'subobj'],
  ...
  <div id="authSection" style="display: none;"><br />
    <div class="ui inverted column stackable center">
      ...
      <div>
        <span class="dbminputlabel">Substituir Object SQL por:</span><br>
        <input class="round" id="subobj" type="text" placeholder="1 ou Teste">
      </div>
        ...
        if (query) {
          sequelize.query(query, { type: Object.keys(Sequelize.QueryTypes).find(type => query.toUpperCase().startsWith(type)) || Sequelize.QueryTypes.RAW }).then((results, metadata) => {
            ...
            if(out == "[object object]" || out == "[]" || out == " " || out == "" || out == null) {
              if(data.subobj.length > 0) {
                out = this.evalMessage(data.subobj, cache);
              }
            }
